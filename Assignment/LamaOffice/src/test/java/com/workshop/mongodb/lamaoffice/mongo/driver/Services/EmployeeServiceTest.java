...
public class EmployeeServiceTest {
    ...
    public void addDepartment() throws Exception {
        ...
        testExceptionDepartment(temp,d);
    ...
    private void testExceptionDepartment(Employee temp,Department d) {
        try {
            service.addDepartment(temp.getId(), new ObjectId());
            fail();
        } catch (ForeignObjectIdNotFound ex) {

        } catch (Exception ex) {
            fail();
        }
        try {
            service.addDepartment(new ObjectId(), d.getId());
            fail();
        } catch (ForeignObjectIdNotFound ex) {
            fail();
        }catch (ObjectIdNotFound ex){
    ...
    public void addProject() throws Exception {
        ...
        assertEquals(1, empRepo.findById(temp.getId()).getProjects().size());
        try {
            service.addProject(temp.getId(), new ObjectId());
            fail();
        } catch (ObjectIdNotFound ex) {
    ...
    public void deleteProject() throws Exception {
        ...
        assertTrue(empRepo.findById(temp.getId()).getProjects().contains(p.getId()));
        assertEquals(1, empRepo.findById(temp.getId()).getProjects().size());
        service.deleteProject(temp.getId(), p.getId());
        assertFalse(empRepo.findById(temp.getId()).getProjects().contains(p.getId()));
    ...
    public void updateDepartment() throws Exception {
        ...
        service.changeDepartment(temp.getId(), newDep.getId());
