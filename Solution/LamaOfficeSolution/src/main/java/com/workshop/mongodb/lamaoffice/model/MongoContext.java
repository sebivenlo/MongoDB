    ...
    public MongoContext(PropertyContext context) {
        ...
        if (!context.getProperties().getProperty("user").isEmpty()) {
            ...
            MongoCredential cred = MongoCredential
                    .createCredential(property.getProperty("user"), property.getProperty("db"),
                            property.getProperty("password").toCharArray());
        ...
        MongoClient mongoClient
                = new MongoClient(new ServerAddress(property.getProperty("host"),
                Integer.parseInt(property.getProperty("port"))), credentials, options);
        ...
        database = mongoClient.getDatabase(property.getProperty("db")).withCodecRegistry(pojoCodecRegistry);
    }
